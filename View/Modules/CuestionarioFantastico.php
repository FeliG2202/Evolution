...
<?php $dates = $cuestionarioFantasticoController->ValideteFechaFantastico();?>

<?php $validate = $cuestionarioFantasticoController->readValidateFantastico(); ?>
<?php if (!$validate) { ?>
	<div class="col-lg-10 mt-5 mb-5 p-4 bg-white rounded shadow-sm mx-auto">
		<form method="POST" class="needs-validation" novalidate>
			<table class="table table-hover table-sm">
				...
				<?php
				$request = $cuestionarioFantasticoController->createCuestionarioFantastico();
				if ($request != null) {
					if ($request[0]) {
					...
					<tr>
						...
						<td>
							<select id="inputState" name="pregunta4" class="custom-select" required>
								<option value="" selected>Seleccione</option>
								<option value="2" value="">casi siempre</option>
								<option value="1" value="">a veces</option>
								<option value="0" value="">casi nunca</option>
							</select>
							<div class="valid-feedback">la respuesta es correcta!</div>
							<div class="invalid-feedback">llenar el campo!</div>
						</td>
					...
					<tr>
						...
						<td>
							<select id="inputState" name="pregunta5" class="custom-select" required>
								<option value="" selected>Seleccione</option>
								<option value="2">3 o mas veces por semana</option>
								<option value="1">1 vez por semana</option>
								<option value="0">no hago nada</option>
							</select>
							<div class="valid-feedback">la respuesta es correcta!</div>
							<div class="invalid-feedback">llenar el campo!</div>
						</td>
					...
					<tr>
						...
						<td>
							<select id="inputState" name="pregunta8" class="custom-select" required>
								<option value="" selected>Seleccione</option>
								<option value="2">Ninguna de éstas</option>
								<option value="1">Algunas de éstas</option>
								<option value="0">todas éstas</option>
							</select>
							<div class="valid-feedback">la respuesta es correcta!</div>
							<div class="invalid-feedback">llenar el campo!</div>
						</td>
					...
					<tr>
						...
						<td>
							<select id="inputState" name="pregunta10" class="custom-select" required>
								<option value="" selected>Seleccione</option>
								<option value="2">0 a 4 Kilos de más</option>
								<option value="1">5 a 8 Kilos de más</option>
								<option value="0">Más de 8 Kilos</option>
							</select>
							<div class="valid-feedback">la respuesta es correcta!</div>
							<div class="invalid-feedback">llenar el campo!</div>
						</td>
					...
					<tr>
						...
						<td>
							<select id="inputState" name="pregunta11" class="custom-select" required>
								<option value="" selected>Seleccione</option>
								<option value="2">No, los últimos 5 años</option>
								<option value="1">No, el último año</option>
								<option value="0">sí, este año</option>
							</select>
							<div class="valid-feedback">la respuesta es correcta!</div>
							<div class="invalid-feedback">llenar el campo!</div>
						</td>
					...
					<tr>
						...
						<td>
							<select id="inputState" name="pregunta13" class="custom-select" required>
								<option value="" selected>Seleccione</option>
								<option value="2">0 a 7 tragos</option>
								<option value="1">8 a 12 tragos</option>
								<option value="0">más de 12 tragos</option>
							</select>
							<div class="valid-feedback">la respuesta es correcta!</div>
							<div class="invalid-feedback">llenar el campo!</div>
						</td>
					...
					<tr>
						...
						<td>
							<select id="inputState" name="pregunta15" class="custom-select" required>
								<option value="" selected>Seleccione</option>
								<option value="2">Nunca</option>
								<option value="1">sólo rara vez</option>
								<option value="0">A menudo</option>
							</select>
							<div class="valid-feedback">la respuesta es correcta!</div>
							<div class="invalid-feedback">llenar el campo!</div>
					...
					<tr>
						...
						<td>
							<select id="inputState" name="pregunta18" class="custom-select" required>
								<option value="" selected>Seleccione</option>
								<option value="2">Menos de 3 al día</option>
								<option value="1">de 3 a 6 al día</option>
								<option value="0">más de 6 al día</option>
							</select>
							<div class="valid-feedback">la respuesta es correcta!</div>
							<div class="invalid-feedback">llenar el campo!</div>
						</td>
...
<?php } else { ?>
	<div class="col-lg-10 mx-auto mt-5 mb-5 bg-white p-4 shadow-sm rounded">
		<form method="POST">
			<h3 class="text-center mt-3">Resultados del Cuestionario fantastico</h3>

			<div class="input-group mb-3 mt-4">
				<select class="custom-select" name="dateValidate" id="inputGroupSelect04" aria-label="Example select with button addon" required>
					<option value="" selected>Seleccione</option>
					<?php foreach($dates as $key => $date) {
						echo('<option value="' . $date['cuestionario_fantastico_fecha_creacion'] . '">'. $date['cuestionario_fantastico_fecha_creacion'] . '</option>');
					} ?>
				</select>
			...
			<?php 
			$suma = $cuestionarioFantasticoController->readResultadoFantastico();
			if ($suma != null) {
				$newValue = intval($suma['cuestionario_fantastico_resultado']);

				if ($newValue > 0 && $newValue <= 46) {
					echo('<div class="alert alert-danger" role="alert">Estás en la zona de peligro, pero la honestidad es tu
						real valor.</div>');
				} elseif ($newValue >= 47 && $newValue <= 72) {
					echo('<div class="alert alert-warning" role="alert">Algo bajo, podrías mejorar.</div>');
				} elseif ($newValue >= 73 && $newValue <= 84) {
					echo('<div class="alert alert-info" role="alert">Adecuado, estás bien.</div>');
				} elseif ($newValue >= 85 && $newValue <= 102) {
					echo('<div class="alert alert-primary" role="alert">Buen trabajo. Estás en el camino correcto.</div>');
				} elseif ($newValue > 102) {
					echo('<div class="alert alert-success" role="alert">Felicitaciones. Tienes un estilo de vida Fantástico.</div>');
				}
			}
			?>
		</form>
